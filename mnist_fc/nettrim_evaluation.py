...
from PrunedFCnet import PrunedFCSoftmaxModel
...
def parallel_nettrim(mnist, epsilon_gain, original_weights, original_biases):
    ...
    pruned_weights = [0] * num_layers
    pruned_biases = [0] * num_layers
...
def fine_tuning(mnist, weights, biases):
...
def compute_performance(mnist, weights, biases):
    nn = BasicFCSoftmaxModel()
...
if __name__ == '__main__':
    ...
    result_folder = 'Z:/data/NetTrim/mnist_fc/exp1/'
    original_model = result_folder + 'original_model.npz'  # original network parameters file

    mnist_db = input_data.read_data_sets(mnist_folder, one_hot=True)

    print('=' * 60)
    print('loading parameters of the model...')
    org_weights, org_biases = load_network_parameters(original_model)
    if org_weights is None:
        print('The model is not trained. Training a new model...')
        org_weights, org_biases = train_regulized_network(mnist_db, l1_weight=l1_regulizer, l2_weight=l2_regulizer,
                                                          keep_prob=keep_p, file_name=original_model)

    org_acc, org_nnz, org_Y = compute_performance(mnist_db, org_weights, org_biases)
    ...
    for eps_gain in epsilon_gains:
        pruned_model = result_folder + 'parallel_{:.3f}.npz'.format(eps_gain)  # pruned model parameters file
        ...
        if nt_weights is None:
            ...
            nt_weights, nt_biases = parallel_nettrim(mnist_db, eps_gain, org_weights, org_biases)
            np.savez_compressed(pruned_model, w=nt_weights, b=nt_biases)

        print('computing performance of the nettrim pruned model...')
        org_acc, org_nnz, org_Y = compute_performance(mnist_db, org_weights, org_biases)
        nt_acc, nt_nnz, nt_Y = compute_performance(mnist_db, nt_weights, nt_biases)
        ...
        hptd_acc, hptd_nnz, hptd_Y = compute_performance(mnist_db, hptd_weights, hptd_biases)

        # =====================================================
        # fine-tuning parameters
        print('=' * 60)
        print('fine-tuning parameters...')
        nt_ft_acc1, nt_ft_acc2, nt_ft_acc3 = fine_tuning(mnist_db, nt_weights, nt_biases)
        hptd_ft_acc1, hptd_ft_acc2, hptd_ft_acc3 = fine_tuning(mnist_db, hptd_weights, hptd_biases)

        total_pruning = 100.0 - np.sum(nt_nnz) * 100.0 / np.sum(org_nnz)
