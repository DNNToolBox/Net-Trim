import time
import numpy as np
from models.BasicFCnet import BasicFCSoftmaxModel
from models.RegularizedFCnet import RegularizedFCSodftmaxModel
...
trained_model = trained_folder + 'FC784_300_1000_300_10.npz'
result_folder = '../data/TrimmedModels/FC784_300_1000_100_10/'
...
def train_network(mnist):
    initial_weights, initial_biases = load_network_parameters(trained_model)
...
def prune_fc_parallel(mnist, epsilon_gain, rho=None, keep_prob=None):
    if rho is None:
    ...
    for g in epsilon_gain:
        ...
        np.savez_compressed(file_name, w=pruned_Weights, b=pruned_biases)


def prune_fc_cascade(mnist, epsilon_gain, rho=None, keep_prob=None):
    if rho is None:
        file_name = trained_model
    else:
        file_name = trained_folder + 'FC784_300_1000_300_10/Params_{0:.4f},{1:.2f}.npz'.format(rho, keep_prob)
    ...
    for f in os.listdir(parameter_folder):
        if fnmatch.fnmatch(f, '*.npz'):
            ...
            if os.path.isfile(file_name):
                ...
                acc = nn.compute_accuracy(mnist.test.images, mnist.test.labels)
                nz = [np.count_nonzero(np.abs(w) > 1e-6) for w in initial_weights]
...
if __name__ == '__main__':
    ...
    # params_folder = trained_folder + 'FC784_300_1000_300_10/'
    ...
    #         prune_fc_parallel(mnist_db, eps_gain_parallel, rho=r, keep_prob=p)
    #
    ...
    #         print('+' * 60)
    #         print('rho={0:.4f}, drop-out prob.={1:.2f}'.format(r, p))
    #         prune_fc_cascade(mnist_db, eps_gain_cascade, rho=r, keep_prob=p)
